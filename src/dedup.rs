...
impl Deduplicator {
    ...
    pub fn insert(&mut self, finding: Finding) -> usize {
        let key = format!("{}:{}", finding.secret_hash, finding.rule_id);
...
mod tests {
    ...
    fn make_finding(value: &str, rule_id: &str, field: &str) -> Finding {
        Finding {
            ...
            issue_url: "https://jira.example.com/browse/TEST-1".into(),
    ...
    fn test_dedup_same_secret_two_fields() {
        ...
        dedup.insert(make_finding("ghp_test123", "github_token", "comment[0].body"));
    ...
    fn test_different_secrets_no_dedup() {
        let mut dedup = Deduplicator::new();
        dedup.insert(make_finding("secret_a", "generic_password_assignment", "desc"));
        dedup.insert(make_finding("secret_b", "generic_password_assignment", "desc"));
